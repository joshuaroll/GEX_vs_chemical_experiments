...
# Imports for fusion model
from fusion_moe_models import *
...
class MultiDCP(nn.Module):
    def __init__(self, device, model_param_registry):
        ...
        self.latent_dim = self.gene_emb_dim + self.drug_emb_dim
        
        # Adjust embed_dim based on fusion type
        if self.fusion_type in ["sparse_moe", "balanced_moe"]:
            self.embed_dim = self.cell_id_emb_dim  # Only cell embeddings
        elif self.fusion_type == "structured_moe":
            self.embed_dim = self.cell_id_emb_dim # PDGrapher has no dosage+ self.pert_idose_emb_dim  # Cell + dosage embeddings
        else:
            self.embed_dim = self.cell_id_emb_dim  # Default for concatenation
        
        # Select the MoE model for fusing cell and drug information
        if self.fusion_type == "sparse_moe":
            self.fusion_layer = SparseMoE(self.latent_dim, self.embed_dim, num_experts=8, k=1, sparsity_coef=1)
        elif self.fusion_type == "balanced_moe":
            self.fusion_layer = BalancedMoE(self.latent_dim, self.embed_dim, num_experts=8, k=2, sparsity_coef=0.1)
        elif self.fusion_type == "structured_moe":
            self.fusion_layer = StructuredSparseMoE(
            ...
            print(f"No MoE fusion_type: {self.fusion_type}")
        
        # Make the MoE balance loss coefficient available to training code
        self.moe_balance_coef = model_param_registry.get('moe_balance_coef', 1.0)
        print(f"Using {self.fusion_type} fusion model with balance coefficient {self.moe_balance_coef}")
        ...
        # Adjust linear_dim based on fusion type
        if self.fusion_type == "structured_moe":
            self.linear_dim = self.latent_dim + self.embed_dim  # MoE output dimension
        else:
            self.linear_dim = self.drug_emb_dim + self.gene_emb_dim + self.cell_id_emb_dim  # Default concatenation
    ...
    def forward(self, input_drug, input_gene, mask, input_cell_gex, epoch=0):
        ...
        drug_embed = drug_embed.unsqueeze(1).repeat(1, self.num_gene, 1)

        ## Gene embedding part
        gene_embed = self.gene_embed(input_gene)
        gene_embed = gene_embed.unsqueeze(0).repeat(num_batch, 1, 1)
        ...
        # pert_idose_embed = self.pert_idose_embed(input_pert_idose).unsqueeze(1).repeat(1, self.num_gene, 1)

        ## Fusion or concatenation
        if self.fusion_type in ["sparse_moe", "balanced_moe", "structured_moe"]:
            if self.fusion_layer is not None:
                batch, num_gene, _ = drug_gene_embed.shape

                # For StructuredMoE: Combine cell + dose before fusion
                if self.fusion_type == "structured_moe":
                    cell_dose_embed = cell_id_embed #torch.cat([cell_id_embed, pert_idose_embed], dim=2)
                    drug_gene_flat = drug_gene_embed.view(-1, self.latent_dim)
                    
                    # change from hard values
                    # Ensure the last dimension of cell_dose_embed is 50
                    #if cell_dose_embed.size(-1) != 50:
                    #    raise ValueError(f"Expected last dimension of cell_dose_embed to be 50, but got {cell_dose_embed.size(-1)}")
                    
                    cell_dose_flat = cell_dose_embed.view(-1, 60)  # Reshape to [batch * num_gene, 50]
                    fused_flat = self.fusion_layer(drug_gene_flat, cell_dose_flat)
                    fused = fused_flat.view(batch, num_gene, -1)
                    drug_gene_embed = fused
                    # For non-structured MoE types: Concatenate dosage after fusion
                else:
                    drug_gene_flat = drug_gene_embed.view(-1, drug_gene_embed.size(-1))
                    cell_dose_flat = cell_id_embed.view(-1, cell_id_embed.size(-1))

                    fused_flat = self.fusion_layer(drug_gene_flat, cell_dose_flat)
                    fused = fused_flat.view(batch, num_gene, -1)
                    drug_gene_embed = fused
                    # For non-structured MoE types: Concatenate dosage after fusion
                    drug_gene_embed = drug_gene_embed
                    # drug_gene_embed = torch.cat((drug_gene_embed, pert_idose_embed), dim=2)

        else:
            # Default concatenation
            # drug_gene_embed = torch.cat((drug_gene_embed, cell_id_embed, pert_idose_embed), dim=2)
            drug_gene_embed = torch.cat((drug_gene_embed, cell_id_embed), dim=2)

        ## Apply ReLU activation and MLP
...
class MultiDCPBase(nn.Module):
    '''
    The bases module for multidcp. The following multiple variants for multidcp are inherited from this module
    '''
    def __init__(self, device, model_param_registry):
...
class MultiDCPOriginal(MultiDCPBase):
    ...
    def forward(self, input_drug, input_gene, mask, input_cell_gex, epoch = 0):
        out = self.multidcp(input_drug, input_gene, mask, input_cell_gex, epoch = epoch)[0]
...
class MultiDCPPretraining(MultiDCPBase):
    ...
    def forward(self, input_drug, input_gene, mask, input_cell_gex):
        out = self.multidcp(input_drug, input_gene, mask, input_cell_gex)[0]
        # out = [batch * num_gene * hid_dim]
        out = self.relu(out)
        # out = [batch * num_gene * hid_dim]
...
class MultiDCPEhillPretraining(MultiDCPBase):
    ...
    def forward(self, input_drug, input_gene, mask, input_cell_gex, job_id = 'perturbed', epoch = 0):

        out, cell_hidden_ = self.multidcp(input_drug, input_gene, mask, input_cell_gex, epoch = epoch)
...
class MultiDCP_AE(MultiDCPBase):
    ...
    def forward(self, input_cell_gex, input_drug = None, input_gene = None, mask = None, job_id = 'perturbed', epoch = 0):
        if job_id == 'perturbed':
            return self.perturbed_trans(input_cell_gex, input_drug, input_gene, mask, epoch)
        else:
            return self.autoencoder(input_cell_gex, epoch)

    def perturbed_trans(self, input_cell_gex, input_drug, input_gene, mask, epoch):
        out, cell_hidden_ = self.multidcp(input_drug, input_gene, mask, input_cell_gex, epoch = epoch)
        # out = [batch * num_gene * hid_dim]
        out = self.relu(out)
        # out = [batch * num_gene * hid_dim]
