...
## Wiring-vs-weights result (2026-06-24) — VERDICT: DEAD-WEIGHTS (D), precise mechanism

`results/tables/P2_wiring_vs_weights.md` + `scripts/diagnose_wiring_vs_weights.py` (commit 00b9f55). Ruled out our wiring and a missing mode; isolated the exact failure point:
- **(W) ruled out:** an upstream-native rebuild (registry + `MultiDCP_CheMoE_AE`, strict-load 0/0) bypassing `region_signature.py` shows the SAME invariance (pred max|Δ| 3.1e-10 for periportal-vs-pericentral). Our wiring is correct — `region_signature.py:779` passes `input_cell_gex=basal` into the arg the graph consumes (multidcp_chemoe_pdg.py L244→257→289), no detach/×0/gate.
- **(M) ruled out:** `MultiDCP_CheMoEBase.forward` (L345) always uses the perturbed head; `job_id` is ignored ("CheMoE doesn't use autoencoder mode"); no AE/decode method exists. No mode routes basal differently.
- **Mechanism (graph trace):** basal is ALIVE out of the input Linear (`cell_id_embed` 10716→200→50, Pearson 0.9996 preserved) and the pos-encoder, then DIES inside `nn.Transformer` (collapses to Pearson 1.000000). Two compounding causes baked into the checkpoint: (1) `repeat(1,1,32)` (multidcp_pdg.py:110-112) makes all 32 d_model channels identical → the transformer's internal LayerNorm discards the per-token magnitude carrying the basal; (2) final `encoder.norm.weight` mean|w| 3.3e-3 (≈2 orders below the rest ~0.2) crushes residual variance. Probe: Pearson −1.0 50-d embeds → cell_hidden max|Δ| 2.19e-08.
- **Weight audit:** params are NOT zero (input Linear mean|w| 0.012–0.087; transformer 0.215; head cell-slice 0.078 ≥ drug-slice 0.071; frac|w|<1e-6 = 0.000). It is a COLLAPSED COMPOSED FUNCTION, not zeroed weights — still class (D): the checkpoint cannot condition on the basal.

**Precise implied fix (the "fix the encoder" path now fully characterized):**
- **F4 (genuine predicted signal) = an UPSTREAM architecture+retrain in the parent `MultiDCP_CheMoE_pdg` project:** retrain with `linear_encoder_flag=True` (use `LinearEncoder`, skip the collapsing transformer) OR replace `repeat(1,1,32)` with a learned `Linear(1→32)`. Small, well-understood change with a known mechanism — BUT it **reverses `DEC-frozen-baseline`** (retraining the upstream) and is parent-project work → needs professor sign-off. This is the only path to a real *predicted* region-resolved signal.
- **F3 (in-scope, claim-changing):** carry an externally-computed basal contrast (`zone_basal − tissue_mean`) as the region feature downstream of the dead encoder — no model change, but it injects the observed basal, so it is NOT a predicted response; report honestly.
- **F2 ruled out** (fine-tune needs healthy-tissue perturbation targets that don't exist).
- **Reframe-to-negative is now very strong:** the finding is a specific, reproducible architectural pathology (`repeat(1,1,32)` + collapsed final LayerNorm) that makes the frozen CheMoE checkpoint ignore cell/tissue context entirely — a clean methods contribution (concept 4), not a vague null.
