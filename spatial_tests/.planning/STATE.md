...
- **⚠ BLOCKER (2026-06-24): `mdcp_env` is broken — smoke could not launch.** Internally-inconsistent deps: setuptools 80.9.0 removed `pkg_resources.declare_namespace` (I pinned it to **79.0.1** — a shared-env change to revert/keep at owner's discretion), then `wandb 0.19.5` needs `protobuf<6` (env has 6.33.5) and `pydantic>=2.6` (env has 2.1.1) → `import pytorch_lightning` fails (`cannot import name 'Deprecated'` from wandb proto). The env drifted since the Jan-17 working run. The F4 command/plan/data/GPUs are all correct; this is purely env health. **Next: repair `mdcp_env` to a consistent set (pin protobuf<6 + pydantic 2.6–3 + setuptools<80, ideally from a known-good lock) — carefully, it is shared — then relaunch the smoke (`02-F4-SCOPE.md` §2 command, `--max_epoch 8`, GPU 1, `best_model_linearenc_smoke.pt`) and run gate 4a.**
