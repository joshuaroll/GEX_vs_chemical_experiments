...
- **DIAGNOSTIC DONE (2026-06-24) → VERDICT: ENCODER-FLAT (inert basal branch).** `results/tables/P2_encoder_diagnostic.md` (commit b00ef98): the row-17 CheMoE checkpoint IGNORES the basal entirely — inputs from Pearson +0.33 to −0.9999 move the output ≤5.96e-08 while the drug branch moves it 0.08–0.13. Inputs genuinely differ (basals Pearson 0.33 human/0.87 mouse, markers survive) — the basal PIPELINE is fine; the model is (drug,dose)-only. This collapses ALL cell/tissue-conditioned conditions (B/C/S-B/S-C), not just spatial. F1 (pipeline) ruled out. **NEXT, before anything heavy: resolve whether the basal branch is inert in the checkpoint WEIGHTS or our `_call_model` WIRING** (run upstream MultiDCP repo's own inference on 2 basals / inspect context-encoder weights) — wiring bug = trivial in-scope fix + milestone salvaged; inert weights = F3 (downstream delta-basal, NOT a predicted signal) or F4 (find a non-collapsed checkpoint; both available ckpts are non-functional: row-17 inert, row-18 collapsed) or revisit `DEC-frozen-baseline`.
