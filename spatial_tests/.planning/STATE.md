...
- **Reframe decision (2026-06-24 gate review):** **FIX THE ENCODER — re-open Phase 2** (option b). See `.planning/phases/02-multidcp-wiring-tox-head/02-REFRAME.md`. Phase 3 stays blocked. **First step is a mandatory root-cause diagnostic** (input-flat vs encoder-flat — do the per-zone basals actually differ + do zonation markers survive the gene space, BEFORE any encoder work), because the fix and its scope hinge on which it is. **Tension to resolve:** "fix the encoder" via retraining conflicts with the locked `DEC-frozen-baseline` (frozen-only) — in-scope fixes are the basal pipeline (if input-flat) or input-representation/different-checkpoint (if encoder-flat); actual fine-tuning needs revisiting that decision with professor sign-off.
