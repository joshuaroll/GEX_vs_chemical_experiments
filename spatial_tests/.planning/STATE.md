...
status: **PHASE 2 HALTED → F4 ENCODER FIX TESTED + REFRAMED (2026-06-25). Halt Gate 3 fired (pericentral r +0.0166). Encoder fix (linear_encoder) retrained 8 + 10 epochs; Gate 4a FAILS identically both times (Pearson(pred_PP,pred_PC) 0.999987 vs <0.999) → VERDICT (b) objective/data limitation: encoder revived (cell_hidden Δ 1.04) but the drug-dominated DE loss collapses it to collinear predicted DE; more epochs won't help. Gate 4b PASSES (DE acc preserved). DO NOT run the full ~8h F4. Spatial predicted-region arm = closed negative (instrumented-negative / F3). USER SCOPE NOTE: refocus on the CORE feature comparison (structure vs expression vs structure+expression); the combined arm is the missing, goal-relevant experiment.**
stopped_at: F4 encoder fix evaluated. Retrained MultiDCP_CheMoE with --linear_encoder_flag (env mdcp_f4, GPU2): 8-epoch smoke (best_model_linearenc_smoke.pt) and 10-epoch (best_model_linearenc_10ep.pt). Gate 4a basal-sensitivity probe identical both runs: cell_hidden max|Δ| 1.0385 (encoder now LIVE vs dead transformer 1.7e-8), but Pearson(pred_PP,pred_PC)=0.999987 FAILS the <0.999 zonal-shape threshold. Best-val DE-Pearson plateaued 0.6690 (same checkpoint epoch both runs). Conclusion: head is architecturally capable but the DE-reconstruction objective never rewards zonal shape → predicted region signal not achievable with this frozen-objective model. Reframe to instrumented-negative; do not commit the 8h retrain. Artifacts: results/tables/P2_gate4a_10ep.md, P2_gate4a_smoke.md, scripts/gate4a_basal_sensitivity.py (GATE4A_CKPT env override). Also: spatial story-EDA gate chain (results/tables/P1_spatial_story_{liver,kidney}.md; visuals/ folders) — liver regional contrast 2.7x noise (WEAK), kidney cortex/medulla 15.6x (STRONG).
last_updated: "2026-06-25T22:15:00.000Z"
...
**Next action:** **REVIEW THE LATENT THREE-WAY COMPARISON, then sweep encoders + add brain (2026-06-26).** The core feature comparison (structure / expression / both, split by organ) is built and pivoted to **latent representations** per the user: structure = pluggable molecular-encoder latent (`src/spatial/structure_encoders.py`: chemberta now, unimol_v1/v2 swappable), expression = MultiDCP `global_features` latent (`scripts/extract_multidcp_latent.py`, gating_network hook, validated). Driver: `scripts/latent_three_way.py --encoder chemberta --gpu 3` (was RUNNING at handoff; results → `results/tables/P1_latent_three_way_chemberta.md`). VALIDATED CAVEAT: global_features' per-drug signal is the 128-d drug block only (cell/dose constant, std 1.8e-8), so the expression arm = MultiDCP's drug encoder; a near-zero +lift means the two encoders carry the same info — report honestly. Immediate steps: (1) check the chemberta result file; (2) encoder sweep — `pip install unimol_tools` then `--encoder unimol_v1/unimol_v2` + `ecfp4`; (3) brain still blocked (SIDER CID-keyed, needs CID→structure + MedDRA SOC); (4) cross-species translatability (scaffolded). MEASURED-expression three-way also done (`P1_three_way_comparison.*`: structure best, measured expr at chance, no +lift) — superseded by latents. Spatial predicted-region arm CLOSED negative (F4 verdict, `P2_gate4a_10ep.md`). Full handoff: `claude_memory/downstream_2026/notes/2026-06-26-RESUME.md`.
