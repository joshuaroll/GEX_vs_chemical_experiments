...
last_activity: 2026-06-30 -- resumed; committed the Jun 29-30 per-organ three-way tox comparison (2 atomic commits) + refreshed this STATE.
progress:
  total_phases: 8
  completed_phases: 2
  total_plans: 11
...
**Next action (2026-06-30):** the core three-way comparison is DONE for liver+kidney and committed (efa8dac substrate, d2efac6 the P4 suite); the finding is a consistent negative — predicted AND measured expression are structure-ceilinged (see frontmatter status for the six-probe breakdown). Three open forks, none started:
1. **Encoder sweep** — everything rides on ChemBERTa. Re-run at least `scripts/stage2_toxicity.py` across UniMol / ECFP4 / MolFormer to test whether "structure wins" is ChemBERTa-specific. Cheapest hardening of the negative; directly answers its one open attack. (`pip install unimol_tools` for UniMol; `src/spatial/structure_encoders.py` is the pluggable-encoder home.)
...
3. **Write up the negative** — consolidate the six P4 probes into the "expression is structure-ceilinged" result.
