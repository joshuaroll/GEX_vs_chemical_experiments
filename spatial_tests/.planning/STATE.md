...
last_activity: 2026-07-01 -- flagged 2 future Tier-2 experiments (STATE forks 7-8): GSEA/pathway representation of the expression arm + E-Hill dose-response outputs into the tox pipeline. Also: wiring diagrams incl. two-tier fig_framework_simple (7cf1c93, b9bbec3). Earlier 2026-07-01: ordinal severity re-test negative (f621edf); tox-tuned E/F halt gate fired (9e2d108); fusion/omics-signal investigation (6c538b5, f4b6a63, fe29dca). 2026-06-30: encoder sweep (08909ae), writeup (6e153a6), lab-meeting package (045f3dd).
progress:
  total_phases: 8
  completed_phases: 2
  total_plans: 11
...
6. **Ordinal severity re-test** — DONE 2026-07-01 (committed f621edf), user-prompted (DILIrank ships a 0-8 SeverityClass we'd been collapsing to binary; user asked to predict the class + give accuracy & model-selection metric tables). All 3 arms predict SeverityClass; structure (chemberta) vs predicted/measured DE vs both; logreg+RF; FULL(0-8) & POS(>=1) cohorts; drug-disjoint 5x3. Tables `P4_severity_class.md` (Table 1 accuracy, Table 2 ordinal metrics: bal-acc/macro-F1/QWK/Spearman/MAE). **NEGATIVE on the ordinal target too:** best model = RF on structure (FULL QWK 0.239/rho 0.331/MAE 2.524; POS QWK 0.166/rho 0.268); predicted DE < structure; MEASURED DE at/below chance (negative QWK/rho); both ties/trails structure. FULL->POS QWK 0.24->0.17 = part of structure's severity signal is the binary is-it-toxic component; the residual 'grade how bad among toxicants' is where measured omics goes negative. Switching binary->ordinal does NOT rescue omics. Only severity-relevant channel still untested = **dose-response / E-Hill potency** (not gene expression; separate model-wiring effort).

### FUTURE EXPERIMENTS — the two untested Tier-2 levers (NOT YET RUN, flagged 2026-07-01 per user)

Both live in "Tier 2" (independent-biology / representation), the only place the omics-vs-structure
negative could still move. Everything in Tier 1 (structure encoders + engine latent/DE/tuned) is
exhausted and structure-ceilinged. See `lab_meetings/figures/fig_framework_simple.png`.

7. **GSEA / pathway-representation of the expression arm.** We only fed raw 978-gene DE (+ PCA30 +
   supervised top-100 genes), all at chance. Never tried a gene-set-enrichment representation:
   project each drug's DE (measured primary, predicted for contrast) onto curated gene sets
   (MSigDB Hallmark + a hepatotox/DILI panel) via ssGSEA / mean-set enrichment → per-pathway scores,
   then structure vs enrichment vs both, binary + severity, drug/scaffold-disjoint. Rationale: a
   pathway can be predictive when no single gene is (coordinated but weak), which top-k selection
   misses but enrichment captures. Caveat: enrichment reorganizes, can't create signal — measured DE
   was at chance even with supervised selection, so this is a second chance, not a guarantee. Needs a
   gene-set collection file (MSigDB) on disk. Cheap once gene sets are available.

8. **E-Hill dose-response curve outputs into the tox pipeline.** The third channel of the prof's
   drawing, never wired into spatial_tests. MODEL_DOSE (E-Hill scalar potency) exists in the sibling
   `multihead_dili/` (chkpt_dose.pt). Add predicted dose-response (potency/efficacy summary across
   cells) as a feature arm (and a dose-response-only baseline), structure vs dose-response vs
   expression vs all, binary + severity. Rationale: potency plausibly tracks SEVERITY (how bad),
   distinct from gene-expression DE; it is the one severity-relevant channel not yet tested. Bigger
   lift: needs the E-Hill model wired in + a dose-response feature builder. See design docs
   `06_three_channel_pipeline.md` conditions G/H and `downstream_MDCPMoE_05052026_addendum_doseresponse.md`.
